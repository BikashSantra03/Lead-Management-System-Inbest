    ...
    try {
        ...
        if (role === Role.SALES_REP) {
            if (
                ...
                logger.error(`Invalid status update attempt for lead ${id}: ${error.message}`, {
                    userId,
                    role,
...
export const assignLead = async (
    ...
    try {
        ...
        if (!salesRep || salesRep.role !== Role.SALES_REP) {
            ...
            logger.error(`Invalid assignment attempt for lead ${id}: ${error.message}`, {
                userId: managerId,
...
export const deleteLead = async (id: string, userId: string, role: Role) => {
    try {
        ...
        if (role !== Role.MANAGER) {
            ...
            logger.error(`Unauthorized delete attempt for lead ${id}: ${error.message}`, {
                userId,
                role,
            });
            throw error;
